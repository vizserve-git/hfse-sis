...
const PREFIX_MATCH_HREFS = new Set(["/attendance/sections"]);
...
export function AttendanceSidebar({ email, role }: { email: string; role: string }) {
  ...
  function isActive(item: NavItem): boolean {
    // /attendance (Dashboard) is exact-match only so deeper routes don't
    // hijack it. /attendance/sections prefix-matches so /attendance/[sectionId]
    // (the daily writer) still highlights "Sections".
    if (item.href === "/attendance") {
      return pathname === "/attendance";
    }
    if (item.href === "/attendance/sections") {
      return (
        pathname === "/attendance/sections" ||
        (pathname.startsWith("/attendance/") &&
          !pathname.startsWith("/attendance/sections") &&
