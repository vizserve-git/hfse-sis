...
| `docs/context/13-sis-module.md`             | SIS module — Phases 1–2 shipped; replacing Directus      |
...
│   ├── (sis)/sis/            ← SIS module: dashboard, students list + detail (4 tabs), discount-codes, audit-log
│   └── api/                  ← all routes (one folder per resource; incl. api/admissions/export, api/p-files/[enroleeNumber]/{upload,revisions}, api/sis/{search,students/[enroleeNumber]/{profile,family/[parent],stage/[stageKey]}})
├── lib/
...
│   ├── sis/                  ← queries.ts — cached list/detail/search/history helpers for the SIS module
...
├── components/sis/            ← student-data-table, cross-ay-search, status-badge, field-grid, enrollment-history-chips, edit-{stage,profile,family}-{dialog,sheet}
├── components/{app,parent,p-files,sis}-sidebar.tsx
├── components/module-switcher.tsx ← admin/superadmin header dropdown: Markbook ↔ P-Files ↔ SIS
├── supabase/
│   ├── migrations/           ← 001_initial_schema → 011_p_file_revisions
...
2. **Roles in `app_metadata.role`** (`teacher | registrar | admin | superadmin | p-file`). No `user_roles` table. `p-file` is module-scoped to `/p-files/*`; `admin` and `superadmin` see both modules via the module switcher. Within P-Files, `admin` is read-only (browse, view files, see history) — only `p-file` officers and `superadmin` can upload / replace.
...
31. **P-Files module is a separate route group `/(p-files)/`** sharing the same app. Three-tier access: `p-file` officers and `superadmin` have full write access (upload / replace); `admin` has read-only access (browse, view, history) via the module switcher; all other roles are blocked at `proxy.ts` / layout. Reads `ay{YY}_enrolment_documents` (already populated by parent uploads via admissions portal). Dashboard at `/p-files` with per-student completeness matrix, student detail at `/p-files/[enroleeNumber]` with upload-on-behalf + revision history, module-scoped audit log at `/p-files/audit-log`. **P-Files is a repository, not a review queue** — document validation (approve/reject) is deferred to the future SIS module. P-Files only writes URLs + metadata + archives prior versions on replace; it never sets `Rejected` status. Upload-on-behalf uses the `parent-portal` Storage bucket matching the admissions portal convention; multiple PDFs merge into one via `pdf-merger-js`; staff uploads auto-status to `'Valid'`. Documents grouped into non-expiring (ID picture, birth cert, educ cert, medical), student-expiring (passport, pass), and parent/guardian (conditional on parent email). Expiring slots collect structured metadata (passport number *or* pass type + expiry date) and write both `ay{YY}_enrolment_documents` and `ay{YY}_enrolment_applications` in one request. See `docs/context/12-p-files-module.md`.
...
33. **Module switcher** — `components/module-switcher.tsx` is a shadcn `Select` rendered in all three dashboard layouts' sticky header (`app/(dashboard)/layout.tsx` + `app/(p-files)/layout.tsx` + `app/(sis)/layout.tsx`). When `canSwitch` is false it renders a plain icon + label lockup. When the current user is `admin` or `superadmin`, it renders a select trigger with a stacked up/down chevron (Vercel-style) that `router.push`es between `/`, `/p-files`, and `/sis`. `p-file` users stay locked to P-Files; `registrar` reaches SIS via the sidebar nav link but never sees the switcher; teachers never reach any of them.
34. **P-Files upload pipeline (dual-table + merge + archive-on-replace).** `POST /api/p-files/[enroleeNumber]/upload` accepts multipart form data: one or more `file` entries, a `slotKey`, optional `note`, and — for expiring slots — either `passportNumber` (for passport slots) or `passType` (for pass slots), plus `expiryDate`. Behavior: 1 file → upload as-is in original extension; 2+ files → validate all are PDFs, merge via `pdf-merger-js`, store as `.pdf`. Size limits: 10 MB per file, 30 MB per request. **When a file already exists at the canonical path, the route first `move`s it to `<prefix>/<enroleeNumber>/<slotKey>/revisions/<iso>.<ext>` and inserts one row in `p_file_revisions` capturing the pre-replacement snapshot (status / expiry / passport number or pass type / optional note / actor); the new file then overwrites the canonical path.** Writes are split across two tables: the file URL + `Status='Valid'` + expiry land on `ay{YY}_enrolment_documents`; passport number / pass type + expiry are also mirrored to `ay{YY}_enrolment_applications` so the enrolment record stays authoritative. Slot → column map lives in `lib/p-files/document-config.ts::SlotMeta`. Audit action: `pfile.upload` (context flags `replaced: true` when a prior file was archived).
...
36. **P-Files revision history (`011_p_file_revisions.sql`).** Each replacement writes one `p_file_revisions` row with `{ay_code, enrolee_number, slot_key, archived_url, archived_path, status_snapshot, expiry_snapshot, passport_number_snapshot, pass_type_snapshot, note, replaced_by_user_id, replaced_by_email, replaced_at}`. Service-role only (RLS deny-all on authenticated); client reads go through `GET /api/p-files/[enroleeNumber]/revisions?slotKey=...`. Surfaced in the UI by `components/p-files/history-dialog.tsx`, triggered from each document card when a file is on file. Append-only: Hard Rule #6 applies. P-Files `DocumentStatus` type excludes `'rejected'` — document validation moves to SIS; the `uploaded` state (rendered as "Pending review") remains for parent-self-serve uploads whose raw status hasn't been validated yet.
37. **SIS module (Sprint 10 Phases 1–3) is a separate route group `/(sis)/`** replacing Directus as the day-to-day admissions admin UI. Access: `registrar`, `admin`, `superadmin`. Sidebar is flat (Dashboard / Students / Discount Codes / Audit Log) so it stays stable as AYs accumulate; the AY scope is a URL-param dropdown (`?ay=AY2026`) instead of a tree node. Routes: `/sis` dashboard, `/sis/students` list (TanStack table + cross-AY search via `/api/sis/search`), `/sis/students/[enroleeNumber]` detail with 4 tabs (Profile / Family / Enrollment / Documents) + enrollment-history chip strip for returning students (matched by `studentNumber` per Hard Rule #4), `/sis/discount-codes` with create / edit / soft-delete actions, `/sis/audit-log` (module-scoped to `action LIKE 'sis.%'`; `/admin/audit-log` now excludes both `pfile.%` and `sis.%`). Reads use `lib/sis/queries.ts` wrapped in `unstable_cache` with `sis:${ayCode}` tag (600s TTL). Phase 2 writes land at three PATCH routes — `/profile` (demographics on applications row), `/family/[parent]` (father/mother/guardian columns), `/stage/[stageKey]` (one of 9 pipeline stages on status row, auto-stamping `*UpdatedDate` / `*UpdatedBy`). Phase 3 adds `POST /api/sis/discount-codes` + `PATCH /api/sis/discount-codes/[id]?op=expire?` (soft delete = endDate = today; no hard delete) plus `PATCH /api/sis/students/[enroleeNumber]/document/[slotKey]` writing `{slotKey}Status = 'Valid' | 'Rejected'` on `ay{YY}_enrolment_documents`. SIS is now the sole writer of `'Rejected'` per the cross-module contract — P-Files re-renders the state via a restored `'rejected'` case in `DocumentStatus`. Rejection reason lives in audit context only (min 20 chars, matching change-request justification). All writes use zod schemas from `lib/schemas/sis.ts` via `safeParse`, write a per-field diff (`context.changes`) to `audit_log` (actions `sis.profile.update` / `sis.family.update` / `sis.stage.update` / `sis.discount_code.{create,update,expire}` / `sis.document.{approve,reject}`), then call `revalidateTag('sis:${ayCode}', 'max')` so the next render sees fresh data. Stable IDs (`enroleeNumber`, `studentNumber`) are not in any schema — the routes 400 if they're sent. Per-student discount *grants* are not SIS's job — the external enrolment portal writes the `discount1/2/3` slot columns directly; SIS only manages the `ay{YY}_discount_codes` catalogue. P-Files cross-link: SIS Documents tab deep-links `/p-files/[enroleeNumber]#slot-{key}` and the P-Files `DocumentCard` adds `scroll-mt-20 target:` styling. See `docs/context/13-sis-module.md`.
